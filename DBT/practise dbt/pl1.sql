...
create trigger tr1 before insert on d for each ROW
BEGIN
	insert into d1 values(NEW.EMPNO, NEW.ENAME, NEW.LOC, current_date(), current_time(), user());
end  $
delimiter ;
...
BEGIN
	insert into d2 values(OLD.EMPNO, OLD.ENAME, OLD.LOC, current_date(), current_time(), user());
end  $
delimiter ;
/*
exit
